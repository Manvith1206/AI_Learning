...
main()
tab1, tab2 = st.tabs(["Chat with Documents", "Performance Metrics"])
...
with tab2:
    ...
    RagPipelineStepToGetCostAndTimeDict = {
        ...
        constants.CONFIG_EMBEDDER: get_pipeline().get_embedder_cost_and_time(),
        constants.CONFIG_RETRIEVER: get_pipeline().get_retriever_cost_and_time(),
        constants.CONFIG_RERANKER: get_pipeline().get_reranker_cost_and_time(),
        constants.CONFIG_EVALUATOR: get_pipeline().get_evaluator_cost_and_time(),
        constants.CONFIG_VECTOR_STORE: get_pipeline().get_vector_store_cost_and_time(),
        constants.CONFIG_LLM: get_pipeline().get_llm_service_cost_and_time()
    }

    def displayUI(key):
        cost, time_taken = RagPipelineStepToGetCostAndTimeDict[key]
        with st.expander(f"📊 {key} - Performance & Cost", expanded=False):
            ...
            cfg = get_pipeline().config_manager.get_config(key)
