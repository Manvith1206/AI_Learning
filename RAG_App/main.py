...
def main():
    ...
    try:
        ...
        jina_api_key = config_manager.get_secret(constants.JINA_RERANKER_API_KEY)
        ...
        UIComponents.set_session_state_variable("pipeline_config", config_manager)
        logger.info("RAG Pipeline initialized successfully.")

    except MissingConfigurationError as e:
        logger.error(f"CRITICAL ERROR: Missing configuration for RAG Pipeline: {e}")
        UIComponents.display_error(f"Application Critical Error: Missing essential configuration. Please check your API keys and config files. Details: {e}")
        ...
        UIComponents.display_error(f"Application Critical Error: Failed to initialize a RAG component. Please check logs and component configurations. Details: {e}")
        return # Stop the app
    except InvalidConfigurationError as e:
        logger.error(f"CRITICAL ERROR: Invalid configuration for RAG Pipeline: {e}")
        UIComponents.display_error(f"Application Critical Error: Invalid configuration detected. Please check your config files. Details: {e}")
        return # Stop the app
    except PipelineError as e:
        logger.error(f"CRITICAL ERROR: A pipeline-level error occurred during RAG Pipeline initialization: {e}", exc_info=True)
        UIComponents.display_error(f"Application Critical Error: A pipeline error occurred during setup. Please check logs. Details: {e}")
        return # Stop the app
    except Exception as e:
        logger.error(f"CRITICAL ERROR: An unexpected error occurred during RAG Pipeline initialization: {e}", exc_info=True)
        UIComponents.display_error(f"Application Critical Error: An unexpected error occurred during setup. Please check logs. Details: {e}")
        return # Stop the app

    # Initialize the main page of the application, passing the pipeline instance
    with UIComponents.display_spinner("Initializing Main Page..."):
